...
public class DotNetRelease implements Release {
    ...
    public DotNetRelease(String projectRoot) {
    ...
    public void processReadme(File readmeFile) throws IOException {
        Util.processFile(readmeFile, (lines, line) -> {
            ...
            } else if (line.startsWith("| " + majorVersion(PREV_VERSION) + "    ")) {
                String newLine = line.replace(PREV_VERSION, RELEASE_VERSION).replace(PREV_VERSION, RELEASE_VERSION).replace(majorVersion(PREV_VERSION), majorVersion(RELEASE_VERSION));
                lines.add(newLine);
            }
            lines.add(line);
            return lines;
